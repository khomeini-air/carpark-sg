package sg.khomeini.carpark.core.dao.repository;
...
public interface CarParkRepository extends JpaRepository<CarParkEntity, Integer> {
    ...
    @Query("SELECT cp FROM CarParkEntity cp WHERE cp.availability.availableLots > 0 ORDER BY ST_Distance_Sphere(point(cp.longitude, cp.latitude), point(:longitude,:latitude))")
